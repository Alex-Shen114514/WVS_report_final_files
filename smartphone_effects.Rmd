...
```{r data-clean}
df_raw <- read_excel(
  "C:/Users/12545/Desktop/Captures/F00013183-WVS_Wave_7_China_Excel_v5.1.xlsx",
  sheet     = 1,
  col_names = TRUE
)
df <- df_raw %>%
...
1. 按 **Ctrl + S** 保存文件。  
2. 在 RStudio 中点击 **Knit → Knit to PDF**。

此时 RStudio 会使用你在 YAML 中指定的 xelatex 引擎，并跳过 pdfcrop，最终生成的 PDF 中就会包含所有 R 代码、`skim(df)` 的输出、回归结果表和交互效应图了。
